...
@interface ApigeeConnection ()
@property (nonatomic, strong) NSString *token;
@property (nonatomic, strong) NSDate *tokenExpirationDate;
@end

@implementation ApigeeConnection

+ (ApigeeConnection *) sharedConnection
{
    static ApigeeConnection *connection = nil;
    
    static dispatch_once_t token;
    dispatch_once(&token, ^{
        connection = [[ApigeeConnection alloc] init];
    });
...
- (id) init {
    if (self = [super init]) {
        self.server = [ApigeeDataClient defaultBaseURL];
    }
    return self;
}

- (ApigeeConnection *) copy
{
    ApigeeConnection *copy = [super copy];
...
- (BOOL) authenticateWithResult:(ApigeeHTTPResult *) result
{

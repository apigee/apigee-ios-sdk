...
void Apigee_protobuf_c_buffer_simple_append (Apigee_ProtobufCBuffer *buffer,
                                      size_t           len,
                                      const unsigned char *data);

/* === stuff which needs to be declared for use in the generated code === */

struct Apigee__ProtobufCEnumValueIndex
{
  const char *name;
...
/* IntRange: helper structure for optimizing
   ...
   array are sorted */
struct Apigee__ProtobufCIntRange
{
  int start_value;
  unsigned orig_index;
...
int Apigee_protobuf_c_int_ranges_lookup (unsigned n_ranges,
                                  Apigee_ProtobufCIntRange *ranges);
...
typedef void (*Apigee_ProtobufCServiceDestroy) (Apigee_ProtobufCService *service);
void
